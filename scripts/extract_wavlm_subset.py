...
def main(args):
    ...
    allowed_ids = set(df_meta['record_id'].astype(str))

    print(f"Loaded {len(allowed_ids)} valid record_ids from metadata.")
    ...
    all_audio_files = list(input_dir.rglob("*.wav"))

    # Keep only the ones whose filename contains a record_id
    audio_files = []
    for p in all_audio_files:
        fname = p.stem  # filename without extension
        # Check which record_ids appear in filename
        if any(rec_id in fname for rec_id in allowed_ids):
...
if __name__ == "__main__":
    ...
    type=str,
    required=True,
    help="CSV with a 'record_id' column. Only audio files containing these IDs will be processed."
    )
