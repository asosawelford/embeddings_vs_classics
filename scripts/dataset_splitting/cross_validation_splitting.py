...
def stratified_dataset_splitting(dataset_path, k_folds, seed=42):
    ...
    dataset = dataset[['subject_id', 'sample_id', 'task_id' , 'site', 'group_id', 'sex', 'age', 'prosody', 'wav2vec', 'phonation', 'articulation', 'glottal']]
    ...
    stratify_fulltrain_test_based_on = dataset['group_id'] + '_' + dataset['task_id'] + '_' + dataset['sex'].map(lambda x: str(x))
    for i, (fulltrain_idx, test_idx) in enumerate(sgkf_fulltrain_test.split(dataset, stratify_fulltrain_test_based_on, dataset.subject_id)):
        ...
        stratify_train_validation_based_on = fulltrain['group_id'].map(lambda x: str(x)) + '_' + fulltrain['task_id']
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset-path', type=str, default='./splits/redlat/dataset.csv')
