...
echo "==================================================="
echo "🚀 STARTING FULL SVM BASELINE EXPERIMENTS"
echo "==================================================="

# Loop over each set of tasks
for TASKS in "${TASK_SETS[@]}"; do
    echo "###################################################"
    echo "Running SVM Task Set: $TASKS"
    echo "###################################################"
    ...
    for PAIR in "CN AD"; do
        ...
        echo "---------------------------------------------------"
        echo "Running SVM Group Comparison: $G1 vs $G2"
        echo "---------------------------------------------------"
        ...
        # # 3. Classic Combined (Concatenated SVM)
        # python src/train_svm.py \
        #     --model_type classic \
        #     --metadata "$META" \
        #     --classic_csv "$CLASSIC" \
        ...
        # # 4. Classic Fusion (Dual-Load Concatenated SVM)
        # python src/train_svm.py \
        #     --model_type classic_fusion \
        #     --metadata "$META" \
        #     --classic_csv "$CLASSIC" \
        #     --tasks $TASKS \
        #     --target_groups $G1 $G2

        # # 5. WavLM (Layer-Averaged SVM)
        # python src/train_svm.py \
        #     --model_type wavlm \
        #     --metadata "$META" \
        #     --embedding_dir "$WAVLM" \
        ...
        # 7. Embedding Fusion (WavLM + RoBERTa Concatenated SVM)
        python src/train_svm.py \
            --model_type fusion \
            --metadata "$META" \
            --embedding_dir "$WAVLM" \
...
echo "🎉 ALL SVM EXPERIMENTS COMPLETED SUCCESSFULLY!"